...
public interface AuthApi {
	...
	@ApiResponses(value = {@ApiResponse(responseCode = "200", description = "Token refreshed"),
			@ApiResponse(responseCode = "401", description = "Invalid or expired token")})
	@PostMapping("/refresh")
	ResponseEntity<AuthResponse> refreshToken(
			@Parameter(description = "Bearer token", required = true, example = "Bearer eyJhbGciOiJ...") @RequestHeader("Authorization") String authHeader);
}
