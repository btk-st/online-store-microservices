...
public class OrderConsumerService {
	...
	public void consume(OrderCreatedEvent event, Acknowledgment acknowledgment) {
		...
		for (OrderCreatedEvent.OrderItemEvent item : event.getItems()) {
			...
			BigDecimal discountMultiplier = BigDecimal.ONE.subtract(item.getSale().divide(BigDecimal.valueOf(100)));

			BigDecimal itemTotalPrice = item.getPrice().multiply(BigDecimal.valueOf(item.getQuantity()))
					.multiply(discountMultiplier);

			OrderEntity orderEntity = OrderEntity.builder().orderId(event.getOrderId()).userId(event.getUserId())
					.productId(item.getProductId()).quantity(item.getQuantity()).price(item.getPrice())
					.sale(item.getSale()).totalPrice(itemTotalPrice).build();
