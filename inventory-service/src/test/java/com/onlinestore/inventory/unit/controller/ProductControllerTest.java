...
class ProductControllerTest {
	...
	void setUp() {
		...
		mockMvc = MockMvcBuilders.standaloneSetup(productController).setControllerAdvice(new GlobalExceptionHandler())
				.build();
	...
	void getAllProducts_shouldReturn200AndProductList() throws Exception {
		...
		ProductResponse response2 = ProductResponse.builder().id(UUID.randomUUID()).name("iPhone")
				.price(new BigDecimal("999.99")).quantity(20).build();

		Mockito.when(productService.getAllProducts()).thenReturn(List.of(productResponse, response2));

		MockHttpServletResponse response = mockMvc
				.perform(MockMvcRequestBuilders.get("/api/products").contentType(MediaType.APPLICATION_JSON))
				.andReturn().getResponse();

		Assertions.assertThat(response.getStatus()).isEqualTo(HttpStatus.OK.value());
		Assertions.assertThat(response.getContentType()).contains(MediaType.APPLICATION_JSON_VALUE);
	...
	void getProductById_shouldReturn200AndProduct_whenProductExists() throws Exception {
		...
		MockHttpServletResponse response = mockMvc.perform(
				MockMvcRequestBuilders.get("/api/products/{id}", productId).contentType(MediaType.APPLICATION_JSON))
				.andReturn().getResponse();
	...
	void getProductById_shouldPropagateNotFoundException() throws Exception {
		...
		MockHttpServletResponse response = mockMvc.perform(MockMvcRequestBuilders.get("/api/products/{id}", productId))
				.andReturn().getResponse();
	...
	void getProductById_shouldReturn400_whenInvalidUUID() throws Exception {
		...
		MockHttpServletResponse response = mockMvc
				.perform(MockMvcRequestBuilders.get("/api/products/{id}", "invalid-uuid")).andReturn().getResponse();

		Assertions.assertThat(response.getStatus()).isEqualTo(HttpStatus.BAD_REQUEST.value());
		Mockito.verify(productService, Mockito.never()).getProductById(Mockito.any());
	}

	@Test
	void createProduct_shouldReturn201AndCreatedProduct() throws Exception {
		Mockito.when(productService.createProduct(Mockito.any(CreateProductRequest.class))).thenReturn(productResponse);

		String requestBody = objectMapper.writeValueAsString(createRequest);

		MockHttpServletResponse response = mockMvc.perform(MockMvcRequestBuilders.post("/api/products")
	...
	void deleteProduct_shouldReturn204() throws Exception {
		...
		MockHttpServletResponse response = mockMvc
				.perform(MockMvcRequestBuilders.delete("/api/products/{id}", productId)).andReturn().getResponse();
	...
	void deleteProduct_shouldReturn404_whenProductNotFound() throws Exception {
		...
		Mockito.doThrow(new ProductNotFoundException(productId)).when(productService).deleteProduct(productId);
	...
	void deleteProduct_shouldReturn400_whenInvalidUUID() throws Exception {
		...
		MockHttpServletResponse response = mockMvc
				.perform(MockMvcRequestBuilders.delete("/api/products/{id}", "invalid-uuid")).andReturn().getResponse();

		Assertions.assertThat(response.getStatus()).isEqualTo(HttpStatus.BAD_REQUEST.value());
		Mockito.verify(productService, Mockito.never()).deleteProduct(Mockito.any());
	}
}
